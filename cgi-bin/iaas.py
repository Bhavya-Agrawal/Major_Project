...
if os_name=="rhel7":
	...
	commands.getstatusoutput('sudo virt-install --graphics vnc,listen=192.168.10.40 ,port=5989 --cdrom /root/Downloads/kali-linux-2.0-amd64.iso --ram {} --vcpu {}  --nodisk --name {} --noautoconsole '.format(os_ram,os_cpu,os_name))

else :
       #Creating a snapshot of the os
       commands.getstatusoutput('sudo virt-install --graphics vnc,listen=192.168.10.40,port=5989 --cdrom /root/Downloads/ubuntu-16.04-desktop-amd64.iso --ram {} --vcpu {}  --nodisk --name {} --noautoconsole '.format(os_ram,os_cpu,os_name))
